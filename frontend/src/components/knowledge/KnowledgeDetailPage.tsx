...
export function KnowledgeDetailPage() {
  ...
  const inflight = (kb?.files ?? []).some((f) => f.status === "pending");
  ...
  async function onSearch(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await searchKnowledgeBase(id, query);
  ...
  if (!kb) {
    return (
      <div>
        <PageHeader title="База знаний" crumbs={[{ label: "Главная", href: "/dashboard" }, { label: "Базы знаний", href: "/knowledge" }, { label: "Коллекция" }]} />
    ...
    <div>
      <PageHeader
        ...
        crumbs={[
          ...
          { label: "Базы знаний", href: "/knowledge" },
          { label: kb.name },
        ]}
        actions={
          <button
            type="button"
            onClick={() =>
              void deleteKnowledgeBase(id).then(() => router.push("/knowledge")).catch((e) => setError(e instanceof Error ? e.message : "Ошибка"))
            }
      ...
      <div className="mb-4 rounded-xl border border-border bg-surface p-4 shadow-sm">
        ...
        <ul className="mt-2 divide-y divide-border text-sm">
          {(kb.files ?? []).map((f) => (
            <li key={f.file_id} className="flex justify-between py-2">
              <span>{f.name || f.file_id}</span>
        ...
        <div className="mt-3">
          ...
          <ul className="mt-1 max-h-32 overflow-auto">
            {files.filter((f) => !attached.has(f.id)).map((f) => (
              <li key={f.id}>
